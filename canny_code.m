...
% 2. The thresholding parameter alfa:
     alfa=0.1;
% Get the initial image lena.png
x = im;               
% figure(1)
% subplot(3,2,1);
% imshow(x);
...
% Thinning (Using interpolation to find the pixels 
% where the norms of gradient are local maximum.)
[n,m] = size(Ibw);
I_temp = zeros(n,m);
for i = 2:n-1
for j = 2:m-1
...
y = 1/(std*sqrt(2*pi)).*exp(-x.^2/(2*std^2));
end
